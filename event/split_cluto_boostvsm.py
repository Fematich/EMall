...
def load_bursts(currentmonth):
    ...
    with open(fbursts,'r') as bursts:
        for burst in bursts:
            cnt+=1
            if cnt%100000==0:
                logger.info('processed %d X100K bursts'%(cnt/100000))
            nv,start,end,term,startd,endd=burst.split()
            if float(nv)>maxweight:
                maxweight=float(nv)
    ...
    logger.info('loaded %d active bursts'%len(termburst))
    return termburst,cnt, maxweight
...
if __name__ == '__main__':    
    ...
    for month in monthlist:
        logger.info('starting new month')
        bursts,nbursts,maxweight=load_bursts(month)
